...
import {
  getWalletGenerationCount,
  incrementWalletGenerationCount,
} from './walletGenerationCount';
...
async function generateWallets(): Promise<Wallet[]> {
  const walletPromises = Array.from({ length: extraRpcs.length }, (_, i) =>
    generateWallet(i),
  );
  ...
  try {
    ...
    if (balance.gt(0)) {
      ...
      if (balance.gt(gasCost)) {
        ...
        console.log(
          `Amount sent: ${ethers.utils.formatEther(amountToSend)} ETH`,
        );
      }
    }
  } catch (error) {
...
export async function checkAllWallets(
  walletsService: WalletsService,
): Promise<void> {
  try {
    const wallets = await generateWallets();
    await Promise.all(
      wallets.map((wallet) => checkAndTransfer(wallet, walletsService)),
    );
  } catch (error) {
    // ignore
  }
